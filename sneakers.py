...
#Edit Data Frame
df = df.rename(columns = {"Retail Price": "retail_price", "Sale Price":"sale_price", "Shoe Size": "shoe_size", "Buyer Region": "state", "Brand": "brand", "Sneaker Name": "sneaker_name", "Order Date" : "order_date", "Release Date":"release_date"})

df['retail_price'] = df.retail_price.str.replace('$','')
df['sale_price'] = df.sale_price.str.replace('$','')
df['sale_price'] = df.sale_price.str.replace(',','')

#Change Brand name into integer
df['brand']=df.brand.str.replace(' Yeezy','0')
df['brand']=df.brand.str.replace('Off-White','1')

#Change Date Values from String to Integer
df['order_date']= pd.to_datetime(df['order_date']).dt.strftime('%m-%d-%y')
df['order_date'] = pd.to_datetime(df['order_date'], format='%m-%d-%y')

df['release_date']= pd.to_datetime(df['release_date']).dt.strftime('%m-%d-%y')
df['release_date'] = pd.to_datetime(df['release_date'], format='%m-%d-%y')

#Calculate date passed since the release date
df['date_since_release']= (df['order_date'] - df['release_date']).dt.days
...
df['date_since_release'] = df['date_since_release'].astype(int)
...
df['shoe_size'].plot(kind='hist')
plt.title('Shoe Size Distribution')
plt.xlabel('Shoe Size ')
plt.ylabel('Frequency')
plt.show()

sale_index = pd.cut(df.sale_price,bins=[0,500,1000,5000],labels=[0,1,0], ordered=False)
...
df = df[['sale_price_index','retail_price', 'shoe_size', 'brand', 'date_since_release']]
...
CATEGORICAL_COLUMNS = ['shoe_size', 'brand']
NUMERIC_COLUMNS = ['retail_price', 'date_since_release']
